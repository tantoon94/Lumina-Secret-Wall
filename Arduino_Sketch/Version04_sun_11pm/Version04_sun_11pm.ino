...
void sendmqtt() {
  ...
  // Find the highest LDR value for each ledValue
  int maxLDRValues[36] = {0}; // Assuming ledValues range from 0 to 35
  int maxLDRIndices[36] = {-1}; // To store the index of the LDR with the highest value for each ledValue

  for (int i = 0; i < numLDRs; i++) {
    for (int j = 0; j < 6; j++) {
      int ledValue = ldrArrays[i][j];
      if (ldrValues[i] > maxLDRValues[ledValue]) {
        maxLDRValues[ledValue] = ldrValues[i];
        maxLDRIndices[ledValue] = i;
      }
    }
  }

  // Send messages only for the highest LDR values
  for (int i = 0; i < numLDRs; i++) {
    if (ldrValues[i] > threshold) {
      for (int j = 0; j < 6; j++) {
        int ledValue = ldrArrays[i][j];
        if (maxLDRIndices[ledValue] == i) {
          int cappedValue = constrain(ldrValues[i], 40, 400);
          int R = map(cappedValue, 40, 400, 0, 255);
          int G = map(cappedValue, 40, 400, 255, 0);
          int B = map(cappedValue, 40, 400, 255, 0);
